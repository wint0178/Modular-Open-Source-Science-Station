/**
 ============================================================================
 Wake-on-Interrupt Landslide Detection System - Version 1.1.6 (WOILD_V1.1.6)
 ============================================================================
UPDATES:
 - v1.1.6 | 2026-08-07 | Dynamic Delay Lookups & Uplink/Downlink Control
  * Replaced static `#define` sleep macros with dynamic lookup arrays `MINIMUM_DELAY_LOOKUP_SEC` (30s, 5m, 1h, 2h) and `TRIP_ALERT_LOOKUP_SEC` (30s, 5m, 10m, 30m)[cite: 1, 2].
  * Converted `#define USE_TRIPWIRE` into a runtime variable `use_tripwire` to enable remote control over security loop monitoring[cite: 1, 2].
  * Expanded NVS storage (`woild_cfg`) to load and persist `use_tripwire`, `minimum_delay_index`, and `trip_alert_index` across deep sleep cycles[cite: 1, 2].
  * Updated 2-byte LoRaWAN downlink processing to decode dynamic tripwire toggle (Byte 1, Bit 3), routine sleep index (Byte 1, Bits 4–5), and tripwire alert index (Byte 1, Bits 6–7)[cite: 2].
  * Restructured Byte 7 of the uplink payload to pack 6-bit temperature (0–63°C), dynamic tripwire status (Bit 6), and a reserved expansion bit (Bit 7)[cite: 2].
  * Standardized tripwire transmission state reporting from `3` to `2` across normal and alert handlers[cite: 1, 2].
...
const uint32_t MINIMUM_DELAY_LOOKUP_SEC[4] = { 30, 300, 3600, 7200 }; // 30s, 5m, 1h, 2h
const uint32_t TRIP_ALERT_LOOKUP_SEC[4]    = { 30, 300, 600, 1800 };  // 30s, 5m, 10m, 30m
...
bool use_tripwire = true; // Default runtime state
uint8_t minimum_delay_index    = 2; // Default: 2 (3600s / 1 hour)
uint8_t trip_alert_index       = 2; // Default: 2 (600s / 10 mins)
...
void loadSystemSettings() {
  ...
  software_threshold_lsb = prefs.getUChar("sw_th", 2);
  duration_index         = prefs.getUChar("dur_idx", 3);
  use_tripwire           = prefs.getBool("use_trip", true); //Load tripwire config
  minimum_delay_index    = prefs.getUChar("del_idx", 2);
  trip_alert_index       = prefs.getUChar("trip_idx", 2);
  prefs.end();
  ...
  Serial.printf("  Use Tripwire Hardware: %s\n", use_tripwire ? "ENABLED" : "DISABLED");
  Serial.printf("  Routine Sleep Delay  : Index %d (%lu sec)\n", minimum_delay_index & 0x03, MINIMUM_DELAY_LOOKUP_SEC[minimum_delay_index & 0x03]);
  Serial.printf("  Tripwire Alert Delay : Index %d (%lu sec)\n\n", trip_alert_index & 0x03, TRIP_ALERT_LOOKUP_SEC[trip_alert_index & 0x03]);
}

void saveSystemSettings(uint8_t new_mot, uint8_t new_sw, uint8_t new_dur_idx, bool new_use_tripwire, uint8_t new_del_idx, uint8_t new_trip_idx) {
  ...
  prefs.putBool("use_trip", new_use_tripwire);
  prefs.putUChar("del_idx", new_del_idx & 0x03);
  prefs.putUChar("trip_idx", new_trip_idx & 0x03);
  prefs.end();

  motion_threshold_lsb   = new_mot;
  software_threshold_lsb = new_sw;
  duration_index         = new_dur_idx & 0x07;
  use_tripwire           = new_use_tripwire;
  minimum_delay_index    = new_del_idx & 0x03;
  trip_alert_index       = new_trip_idx & 0x03;
  ...
  if (wakeup_reason == ESP_SLEEP_WAKEUP_EXT1) {
    if (use_tripwire && ((ext1_status & (1ULL << TRIP_PIN)) != 0)) {
      ...
      if (digitalRead(TRIP_PIN) == HIGH) {
        ...
        enterDeepSleep(MINIMUM_DELAY_LOOKUP_SEC[minimum_delay_index & 0x03]);
      }
    } else {
...
void handleAlertMode() {
  ...
  if (use_tripwire && isTripwireBroken) transmissionState = 2; // 2 = Tripwire
...
void handleNormalMode() {
  ...
  if (use_tripwire && isTripwireBroken) transmissionState = 2; 
  ...
  sendSensorData(transmissionState, max_delta_mg);
  uint32_t sleep_sec = (use_tripwire && isTripwireBroken) 
    ? TRIP_ALERT_LOOKUP_SEC[trip_alert_index & 0x03] 
    : MINIMUM_DELAY_LOOKUP_SEC[minimum_delay_index & 0x03];
...
void sendSensorData(uint8_t systemState, float max_delta_mg) {
  ...
  // Map MPU Temp: Constrain to 0°C to 63°C into 6 bits (Bits 0-5)
  uint8_t packed_temp = (uint8_t)constrain(round(raw_temp), 0.0, 63.0) & 0x3F;

  // Prepare Configuration Flags for Bits 6 and 7
  uint8_t use_tripwire_flag = use_tripwire ? 1 : 0; // Bit 6
  uint8_t reserved_bit7     = 0;                    // Bit 7 reserved for future expansion

  // Construct Byte 7
  uint8_t byte7_packed = packed_temp | (use_tripwire_flag << 6) | (reserved_bit7 << 7);
  ...
  uplinkData[7] = byte7_packed; // Byte 7: Temp (Bits 0-5) | use_tripwire (Bit 6) | Reserved (Bit 7)
...
if (lenDown >= 2) {
    ...
    bool new_use_tripwire    = ((byte1 >> 3) & 0x01) == 1; // Bit 3 of Byte 1
    uint8_t new_delay_idx    = (byte1 >> 4) & 0x03; // Bits 4 & 5
    uint8_t new_trip_idx     = (byte1 >> 6) & 0x03; // Bits 6 & 7
    ...
    Serial.printf("-> New Motion Duration Index : %d (%d ms)\n", new_duration_idx, DURATION_LOOKUP_MS[new_duration_idx & 0x07]);
    Serial.printf("-> New Dynamic Tripwire State: %s\n", new_use_tripwire ? "ENABLED" : "DISABLED");
    Serial.printf("-> New Routine Delay Index   : Index %d (%lu sec)\n", new_delay_idx, MINIMUM_DELAY_LOOKUP_SEC[new_delay_idx & 0x03]);
    Serial.printf("-> New Tripwire Delay Index  : Index %d (%lu sec)\n", new_trip_idx, TRIP_ALERT_LOOKUP_SEC[new_trip_idx & 0x03]);
    
    saveSystemSettings(new_motion_lsb, new_software_lsb, new_duration_idx, new_use_tripwire, new_delay_idx, new_trip_idx);
  } else if (lenDown == 1) {
    Serial.println(F("[DOWNLINK WARNING] Received 1 byte, expected 2 bytes (Byte0: Thresh, Byte1: Duration/Delays)."));
  } else {
